...
@implementation RCTVLCPlayer
{
    ...
    int _textTrack; // Store desired text track
    ...
    if (_player) {
        ...
        switch (state) {
            ...
            case VLCMediaPlayerStatePlaying:
                ...
                if (_textTrack == -1 && _player.numberOfSubtitlesTracks > 0) {
                    [_player setCurrentVideoSubTitleIndex:-1];
                    NSLog(@"Force disabled embedded subtitles in Playing state");
                } else if (_textTrack != -1) {
                    [_player setCurrentVideoSubTitleIndex:_textTrack];
                    NSLog(@"Applied text track %d in Playing state", _textTrack);
                }

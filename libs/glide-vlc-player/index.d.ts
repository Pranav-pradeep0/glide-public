...
export interface VLCPlayerSource {
  ...
  initOptions?: string[];
  /**
   * Media options to pass to the player for this specific media
  ...
  isNetwork?: boolean;
  /**
   * Whether to autoplay the media when loaded
   */
  autoplay?: boolean;
}
...
export type VLCSeekEventProps = {
  /**
   * Current playback time after seek
   */
  currentTime: number;
  ...
  /**
   * Current playback position after seek (0-1)
   */
  position: number;
};
...
export type VLCPlayerCallbackProps = {
   ...
   * @param event - Event properties
   */
  onSeek?: (event: VLCSeekEventProps) => void;
};
...
export type VLCPlayerProps = VLCPlayerCallbackProps & {
  ...
  /**
   * Set equalizer gains (dB) for 10 bands.
   ...
   * Positive value delays audio (audio is later than video).
   * Negative value advances audio (audio is earlier than video).
   * @default 0
   */
  audioDelay?: number;
   ...
   * Positive value delays subtitle.
   * Negative value advances subtitle.
   * @default 0
   */
  subtitleDelay?: number;

  /**
   * Set the title of the media for the notification and lock screen controls
   */
  title?: string;

  /**
   * Set the artist of the media for the notification and lock screen controls
   */
  artist?: string;
};
